...
if namespace.hexpath:
    ...
    ser.port = namespace.com
    ser.baudrate = namespace.baudrate
    ser.rts = True
    ser.open()
    # RTS соединена с входом сброса, при открытии порта упадёт в ноль
    time.sleep(0.1) # Задержка на сброс
    ser.rts = False; # Поднимаем уровень 
