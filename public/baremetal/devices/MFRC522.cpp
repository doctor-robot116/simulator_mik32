...
#define MFRC522_CS_PIN      2
#define MFRC522_RST_PIN     8
#define MFRC522_IRQ_PIN     9
#define MFRC522_MOSI_PIN    1
#define MFRC522_SCK_PIN     2
#define MFRC522_MISO_PIN    0
#define MFRC522_NSS_PIN     3
...
// SPI
// буфер для отправки. размер FIFO плюс байт для номера регистра
static uint8_t mfrc522_write_buffer[FIFO_SIZE + 1];
// буфер для приёма
static uint8_t mfrc522_read_buffer[FIFO_SIZE + 1];

// таймаут на передачу по SPI в миллисекундах
uint32_t g_spi_timeout;
uint32_t g_last_spi_size = 0;

static void spi_calc_timeout( uint32_t a_size ) {
...
// начать передачу a_size байтов из mfrc522_write_buffer через SPI с участием DMA
// принятое складываем в mfrc522_read_buffer
static void spi_write_start( uint32_t a_size ) {
  //
  DMA_CONFIG->CONFIG_STATUS = 0x3F + DMA_CONFIG_CURRENT_VALUE_M;
  // DMA[0] - передача SPI1
  DMA_CONFIG->CHANNELS[0].LEN = a_size - 1u;
  DMA_CONFIG->CHANNELS[0].SRC = (uint32_t)mfrc522_write_buffer;
  // DMA[1] - приём SPI1
  DMA_CONFIG->CHANNELS[1].LEN = a_size - 1u;
  DMA_CONFIG->CHANNELS[1].DST = (uint32_t)mfrc522_read_buffer;
  ...
  //
  spi_calc_timeout( a_size );
}


// ждать завершения передачи, запущенной в ST7735_spi_write_start()
...
static void spi_write( uint32_t a_size ) {
  spi_write_start( a_size );
  spi_write_end();
}


static void spi_select() {
  GPIO_0->CLEAR = (1 << MFRC522_CS_PIN);
}

static void spi_release() {
  GPIO_0->SET = (1 << MFRC522_CS_PIN);
}
...
/////////////////////////////////////////////////////////////////////////////////////
MFRC522::MFRC522() {
	// это просто индикатор вызова конструктора, можно убрать
	printf( "-> '%s'\n", __PRETTY_FUNCTION__ );
  // включение DMA
  PM->CLK_AHB_SET = PM_CLOCK_AHB_DMA_M;
  // PAD_CONFIG включен в main.c
  PM->CLK_APB_P_SET = PM_CLOCK_APB_P_GPIO_0_M | PM_CLOCK_APB_P_GPIO_1_M | PM_CLOCK_APB_P_SPI_1_M;
  // PORT0.2 - GPIO, выход (SPI CS)
  PAD_CONFIG->PORT_0_CFG = (PAD_CONFIG->PORT_0_CFG & ~(PAD_CONFIG_PIN_M(MFRC522_CS_PIN)));
  PAD_CONFIG->PORT_0_DS = (PAD_CONFIG->PORT_0_DS & ~(PAD_CONFIG_PIN_M(MFRC522_CS_PIN)))
                        | PAD_CONFIG_PIN(MFRC522_CS_PIN, 2)
                        ;
  GPIO_0->DIRECTION_OUT = (1 << MFRC522_CS_PIN);
  GPIO_0->SET = (1 << MFRC522_CS_PIN);
  ...
  PAD_CONFIG->PORT_1_CFG = (PAD_CONFIG->PORT_1_CFG & ~( PAD_CONFIG_PIN_M(MFRC522_IRQ_PIN)
                                                      | PAD_CONFIG_PIN_M(MFRC522_MISO_PIN)
                                                      | PAD_CONFIG_PIN_M(MFRC522_RST_PIN)
                                                      | PAD_CONFIG_PIN_M(MFRC522_SCK_PIN)
                                                      | PAD_CONFIG_PIN_M(MFRC522_NSS_PIN)
                                                      | PAD_CONFIG_PIN_M(MFRC522_MOSI_PIN) ))
                         | PAD_CONFIG_PIN(MFRC522_SCK_PIN, 1)
                         | PAD_CONFIG_PIN(MFRC522_MISO_PIN, 1)
                         | PAD_CONFIG_PIN(MFRC522_MOSI_PIN, 1)
                         | PAD_CONFIG_PIN(MFRC522_NSS_PIN, 1)
                         ;
  PAD_CONFIG->PORT_1_DS = (PAD_CONFIG->PORT_1_DS & ~( PAD_CONFIG_PIN_M(MFRC522_IRQ_PIN)
                                                    | PAD_CONFIG_PIN_M(MFRC522_RST_PIN) ))
                        ;
  ...
  PAD_CONFIG->PORT_1_PUPD = (PAD_CONFIG->PORT_1_PUPD & ~(PAD_CONFIG_PIN_M(MFRC522_NSS_PIN)))
                        | PAD_CONFIG_PIN(MFRC522_NSS_PIN, 1)
                        ;
  // выход сброса в ноль
  GPIO_1->DIRECTION_OUT = (1 << MFRC522_RST_PIN);
  GPIO_1->CLEAR = (1 << MFRC522_RST_PIN);
  //
  GPIO_1->DIRECTION_IN = (1 << MFRC522_IRQ_PIN);
  // настройка SPI1
  ...
  SPI_1->CONFIG = SPI_CONFIG_MANUAL_CS_M // ручное управление сигналом CHIP_SELECT
                ...
                | SPI_CONFIG_BAUD_RATE_DIV_8_M // частота интерфейса 4 МГц
  ...
  DMA_CONFIG->CHANNELS[0].SRC = (uint32_t)&mfrc522_write_buffer[0]; // пишем всегда из буфера передачи
  ...
  DMA_CONFIG->CHANNELS[1].DST = (uint32_t)&mfrc522_read_buffer[0]; // записываем принятое в буфер приёма
...
void MFRC522::PCD_WriteRegister(	PCD_Register reg,	///< The register to write to. One of the PCD_Register enums.
	...
	mfrc522_write_buffer[0] = reg;
	mfrc522_write_buffer[1] = value;
  spi_select();
  spi_write( 2u );
  spi_release();
 ...
 */
void MFRC522::PCD_WriteRegister(	PCD_Register reg,	///< The register to write to. One of the PCD_Register enums.
									uint32_t count,			///< The number of bytes to write to the register
									uint8_t *values		///< The values to write. Byte array.
								) {
	mfrc522_write_buffer[0] = reg;
	memcpy( &mfrc522_write_buffer[1], values, count );
	spi_select();
	spi_write( count + 1u );
	spi_release();
} // End PCD_WriteRegister()
...
uint8_t MFRC522::PCD_ReadRegister(	PCD_Register reg	///< The register to read from. One of the PCD_Register enums.
	...
	mfrc522_write_buffer[0] = reg | 0x80; // MSB == 1 is for reading. LSB is not used in address. Datasheet section 8.1.2.3.
	mfrc522_write_buffer[1] = 0; // типа стоп-сигнал ;)
	spi_select();
	spi_write( 2u );
	spi_release();
	return mfrc522_read_buffer[1];
...
void MFRC522::PCD_ReadRegister(	PCD_Register reg,	///< The register to read from. One of the PCD_Register enums.
								...
								int rxAlign		///< Only bit positions rxAlign..7 in values[0] are updated.
								) {
	if ( 0 == count ) {
		return;
	}
	// первый байт - адрес регистра, остальные count - 1 байтов - тот же адрес для продолжения чтения
	memset( &mfrc522_write_buffer[0], reg | 0x80, count );
	mfrc522_write_buffer[count] = 0; // стоп-сигнал в последнем передаваемом байте
	spi_select();
	spi_write( count + 1u );
	spi_release();
	// если запрошена коррекция первого принятого байта (нулевой - ответ на передачу адреса регистра)
	if ( 0 != rxAlign ) {
		mfrc522_read_buffer[1] &= (0xFF << rxAlign); // Only update bit positions rxAlign..7 in values[0]
	}
	memcpy( values, &mfrc522_read_buffer[1], count );
} // End PCD_ReadRegister()
...
MFRC522::StatusCode MFRC522::PCD_CalculateCRC(	uint8_t *data,		///< In: Pointer to the data to transfer to the FIFO for CRC calculation.
												uint32_t length,	///< In: The number of bytes to transfer.
	...
	// the operation.
	const uint32_t v_from = g_milliseconds;
	...
	while (((uint32_t)(g_milliseconds - v_from)) < 90u);
...
bool MFRC522::PCD_PerformSelfTest() {
	...
	uint8_t result[FIFO_SIZE];
	PCD_ReadRegister(FIFODataReg, FIFO_SIZE, result, 0);
	...
	if ( 0 != memcmp( reference, result, FIFO_SIZE ) ) {
		return false;
	}
...
void MFRC522::PCD_SoftPowerUp(){
	...
	const uint32_t v_from = g_milliseconds; // create timer for timeout (just in case) 
	
	while ( ((uint32_t)(g_milliseconds - v_from) < 500) ) { // set timeout to 500 ms 
...
MFRC522::StatusCode MFRC522::PCD_TransceiveData(	uint8_t *sendData,		///< Pointer to the data to transfer to the FIFO.
													uint32_t sendLen,		///< Number of uint8_ts to transfer to the FIFO.
...
MFRC522::StatusCode MFRC522::PCD_CommunicateWithPICC(	uint8_t command,		///< The command to execute. One of the PCD_Command enums.
	...
	while ( ((uint32_t)(g_milliseconds - v_from)) < 36 );
...
MFRC522::StatusCode MFRC522::MIFARE_Read(	uint8_t blockAddr, 	///< MIFARE Classic: The block (0-0xff) number. MIFARE Ultralight: The first page to return data from.
											...
											uint8_t *bufferSize	///< Buffer size, at least 18 bytes. Also number of uint8_ts returned if STATUS_OK.
										) {
	MFRC522::StatusCode result;
...
MFRC522::StatusCode MFRC522::MIFARE_Write(	uint8_t blockAddr, ///< MIFARE Classic: The block (0-0xff) number. MIFARE Ultralight: The page (2-15) to write to.
											...
											uint8_t bufferSize	///< Buffer size, must be at least 16 bytes. Exactly 16 uint8_ts are written.
										) {
	MFRC522::StatusCode result;
...
MFRC522::StatusCode MFRC522::MIFARE_Ultralight_Write(	uint8_t page, 		///< The page (2-15) to write to.
														...
														uint8_t bufferSize	///< Buffer size, must be at least 4 bytes. Exactly 4 uint8_ts are written.
													) {
	MFRC522::StatusCode result;
...
bool MFRC522::MIFARE_OpenUidBackdoor(bool logErrors) {
	...
	if ( STATUS_OK != PICC_HaltA() ) { // 50 00 57 CD
		printf( "PICC_HaltA() error\n" );
		return false;
	}
	...
	MFRC522::StatusCode status = PCD_TransceiveData(&cmd, 1, response, &received, &validBits, 0, false); // 40
	if(status != STATUS_OK) {
		if(logErrors) {
	...
	cmd = 0x43;
	validBits = 8;
	status = PCD_TransceiveData(&cmd, 1, response, &received, &validBits, 0, false); // 43
	if(status != STATUS_OK) {
		if(logErrors) {
...
//			  PICC_WakeupA(atqa_answer, &atqa_size);
			...
			status = PCD_Authenticate(MFRC522::PICC_CMD_MF_AUTH_KEY_A, 1, &key, &uid);
			if (status != STATUS_OK) {
...
bool MFRC522::MIFARE_UnbrickUidSector(bool logErrors) {
	...
	// Write modified block 0 back to card
	MFRC522::StatusCode status = MIFARE_Write(0, block0_buffer, 16);
	if (status != STATUS_OK) {
		if (logErrors) {
			printf("MIFARE_Write() failed: %s\n", GetStatusCodeName(status));
		}
