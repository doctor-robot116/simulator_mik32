...
void main() {
  ...
  g_rc522.PCD_Init();
  g_rc522.PCD_DumpVersionToSerial();
  //
  for (;;) {
    if ( g_rc522.PICC_IsNewCardPresent() ) {
      // мигаем светодиодом
      GPIO_2->SET = (1 << 7); // включим светодиод
      printf( "New card\n" );
      if ( g_rc522.PICC_ReadCardSerial() ) {
        g_rc522.PICC_DumpToSerial( &g_rc522.uid );
      }
      GPIO_2->CLEAR = (1 << 7); // выключим светодиод
      delay_ms( 1000u );
    } else {
      delay_ms( 100u );
    }
