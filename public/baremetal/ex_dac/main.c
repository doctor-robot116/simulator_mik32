...
// DDS генератор с опорной частотой 50 кГц
#define MIN_FREQ  21474837u // примерно 250 Гц
#define MAX_FREQ  64424510u // примерно 750 Гц
#define STEP_FREQ 85900 // шаг изменения частоты примерно 1 Гц


// "фаза" генератора
uint32_t g_phase = 0;
// "частота" генератора = (50000 * g_freq) / (2^32)
volatile uint32_t g_freq = MIN_FREQ;
...
void main() {
  ...
  // на выходе DAC0 "синус" с качающейся частотой от 250 до 750 Гц
  // на выходе DAC1 - то же самое, но сдвинутое по фазе на 180 градусов
  for (;;) {
    // повышаем частоту от MIN_FREQ до MAX_FREQ
    for ( uint32_t i = MIN_FREQ; i < MAX_FREQ; i += STEP_FREQ ) {
      g_freq = i;
      delay_ms( 4u );
    }
    // мигаем светодиодом
    GPIO_2->OUTPUT ^= (1 << 7);
    // понижаем частоту от MAX_FREQ до MIN_FREQ
    for ( uint32_t i = MAX_FREQ; i > MIN_FREQ; i -= STEP_FREQ ) {
      g_freq = i;
      delay_ms( 4u );
    }
    // мигаем светодиодом
...
bool process_irqs(void) {
  ...
  if ( 0 != (EPIC->RAW_STATUS & EPIC_LINE_M(EPIC_LINE_TIMER32_0_S)) ) {
    ...
    ANALOG_REG->DAC0.VALUE = g_cos_table_dac[g_phase>>22];
    ANALOG_REG->DAC1.VALUE = g_cos_table_dac[(g_phase + 0x80000000u)>>22]; // сдвинутое по фазе на 180 градусов ((2^32)/4)
    g_phase += g_freq;
