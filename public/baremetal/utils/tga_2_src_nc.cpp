...
int main( int argc, char ** argv ) {
  ...
  for ( int k = 0; k < v_pixels_count; ++k ) {
    uint16_t v_color = (uint16_t)(((v_ptr[2] >> 3) << 11) | ((v_ptr[1] >> 2) << 5) | (v_ptr[0] >> 3));
    // save as BE
    *v_rgb++ = v_color >> 8;
    *v_rgb++ = v_color & 0xFF;
    v_ptr += v_pixel_bytes;
  ...
  ::fprintf( v_fp_out_h.get(), "#ifndef _I%s_\n"
                               ...
                               "#define I%s_width %d\n"
                               "#define I%s_height %d\n\n"
                               "extern const uint8_t I%s_rgb16[%d];\n\n"
                               "#endif\n"
                             , v_header_name.c_str()
                             , v_header_name.c_str()
                             , v_base_name.c_str(), v_tga_head.width
                             , v_base_name.c_str(), v_tga_head.height
                             , v_base_name.c_str(), v_rgb16_bytes
                             );
  // write out file.c
  ::fprintf( v_fp_out_c.get(), "#include \"%s\"\n\n\n"
                               "const uint8_t I%s_rgb16[%d] = {\n"
                             , argv[2]
                             , v_base_name.c_str(), v_rgb16_bytes
                             );
  v_ptr = v_tga_pixels.get();
  int v_line_bytes = 0;
  for ( int k = 0; k < v_rgb16_bytes; ++k ) {
