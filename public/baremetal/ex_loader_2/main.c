...
void process_init_mcu() {
  // включаем тактирование GPIO_0, GPIO_2, PAD_CONFIG, UART_0, SPIFI, DMA, CRC32
  ...
  PM->CLK_AHB_SET = PM_CLOCK_AHB_SPIFI_M | PM_CLOCK_AHB_DMA_M | PM_CLOCK_AHB_CRC32_M;
  // PORT2.6 - GPIO
  // PORT2.0-PORT2.5 - SPIFI (sck, cs, D0, D1, D2, D3)
  PAD_CONFIG->PORT_2_CFG = (PAD_CONFIG->PORT_2_CFG & ~(0x3FFF))
                         | PAD_CONFIG_PIN(0, 1)
                         | PAD_CONFIG_PIN(1, 1)
                         | PAD_CONFIG_PIN(2, 1)
                         | PAD_CONFIG_PIN(3, 1)
                         ...
                         | PAD_CONFIG_PIN(5, 1)
                         ;
  PAD_CONFIG->PORT_2_DS = (PAD_CONFIG->PORT_2_DS & ~(0x3FFF));
  PAD_CONFIG->PORT_2_PUPD = (PAD_CONFIG->PORT_2_PUPD & ~(0x3FFF))
...
void go_to_application() {
  ...
  // восстановление режима вывода GPIO2.6
  PAD_CONFIG->PORT_2_PUPD &= ~PAD_CONFIG_PIN_M(6);
  PAD_CONFIG->PORT_2_DS &= ~PAD_CONFIG_PIN_M(6);
  // отключение UART_0
  UART_0->CONTROL1 = 0;
  UART_0->FLAGS = 0xFFFFFFFF;
  UART_0->DIVIDER = 0;
...
uint32_t start_read_DMA_UART( uint8_t * a_dst, uint32_t * a_size_remains ) {
  ...
  DMA_CONFIG->CHANNELS[0].SRC = (uint32_t)&UART_0->RXDATA; // читаем из регистра принятых данных
  ...
  DMA_CONFIG->CHANNELS[0].CFG = DMA_CH_CFG_READ_MODE_PERIPHERY_M // читаем из периферийного устройства
                              ...
                              | (DMA_UART_0_INDEX << DMA_CH_CFG_READ_REQUEST_S) // номер для UART_0
                              ;
...
bool firmware_update() {
  ...
  for (;;) {
    ...
    if ( !mik32_gets( 501u, (uint8_t *)&v_header, sizeof(v_header) ) ) {
      // нет, тогда проверяем состояние кнопки на PORT2.6 - если нажата (высокий уровень) - то продолжаем ожидать данные
      ...
      if ( v_header.m_size > SPIFI_FLASH_SIZE ) {
        ...
        UART_0->CONTROL3 |= UART_CONTROL3_DMAR_M;
      }
      // два канала DMA
      // 0. на чтение из UART_0
      // 1. на запись в SPIFI
